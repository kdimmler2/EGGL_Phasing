...
rule all:
	input:
		expand('phasing/unphased/chr{chrom}/output.vcf.gz', chrom=[f'{i}' for i in range(1,32)] + ['X']),
		expand('phasing/unphased/chr{chrom}/output.vcf.gz.tbi', chrom=[f'{i}' for i in range(1,32)] + ['X']),
        expand('phasing/phased/chr{chrom}/output.vcf.gz', chrom=[f'{i}' for i in range(1,32)] + ['X']),
        expand('phasing/phased/chr{chrom}/output.vcf.gz.tbi', chrom=[f'{i}' for i in range(1,32)] + ['X']),
...
rule phase:
    ...
    shell:
        '''
            java -jar ../../repository/beagle.r1399.jar \
                gt={input.split_vcf} \
...
rule vcf_list:
    input:
        vcfs = expand('phasing/phased/{chrom}/output.vcf.gz', chrom=[f'chr{i}' for i in range(1,32)] + ['chrX'])
    output:
    ...
    run:
        ...
        chrom=[f'chr{i}' for i in range(1,32)] + ['chrX']
        for chrm in chrom:
